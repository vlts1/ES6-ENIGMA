...
let userID = Math.random().toString(16).slice(5);
...
$(document).ready(function() {
    $('#sidebar-btn').on('click', function() {
        ...
        $('#send_to_server').toggleClass('pushed');
    });
...
function displayFilesFromServer() {
    window.open('http://localhost:3000/output/' + userID, '_blank').focus();
}
...
function sendFilesToServer() {
    for (let fileID = 0; fileID < openFiles.length; fileID++) {
        ...
        $.ajax({
            url : 'http://localhost:3000/files/' + userID + '/' + file.name,
            type: "POST",
            data: {'content': file.content},
            success: function (result) {
                console.log(result);
                if (fileID === openFiles.length - 1) {
